...
int D_Add(Table* tab) {
	...
	strinfo = getstr();
	//scanf("%s",&strinfo);
	Item* item = create_item(num1, num2, strinfo);
	InfoType* info = create_info(num1, num2, strinfo);
	int res = insert(tab, key1, key2, item, info);
	free(info);
	if (res == SUCCESS)
...
int D_Find(Table* tab) {
	...
	int u = NOT_FOUND;
	b = get_int(&a);
	Table result = {1,1,0,NULL,NULL,NULL, NULL };
...
int D_Show(Table* tab) {
	printf("KeySpace1:\n");
	printf("{key1, key2(release), num1, num2, offset, len}\n");
	for (int i = 0; i < tab->msize1; i++) {
		if (tab->ks1[i].item == NULL)
			...
			print_info(tab, tab->ks1[i].item->offset, tab->ks1[i].item->len); 
		}
	...
	for (int i = 0; i < tab->msize2; i++) {
		if (tab->ks2[i].item == NULL)
			...
			print_info(tab, tab->ks2[i].item->offset, tab->ks2[i].item->len);
		}
